...
local Path = require("plenary.path") -- Import the plenary.path module
local data_path = vim.fn.stdpath("data") -- Get the standard data path for Neovim, typically ~/.local/share/<NVIM_APPNAME>
local save_folder = Path:new(data_path, "codecompanion_chats") -- Create a new path object for the save folder
if not save_folder:exists() then -- Check if the save folder does not exist
  save_folder:mkdir({ parents = true }) -- Create the save folder, including any necessary parent directories
end

-- Telescope picker for our saved chats
vim.api.nvim_create_user_command("CodeCompanionLoad", function() -- Create a new user command "CodeCompanionLoad"
  local t_builtin = require("telescope.builtin") -- Import the telescope.builtin module
  local t_actions = require("telescope.actions") -- Import the telescope.actions module
  local t_action_state = require("telescope.actions.state") -- Import the telescope.actions.state module

  local function start_picker() -- Define a function to start the telescope picker
    t_builtin.find_files({ -- Use the find_files function from telescope.builtin
      prompt_title = "Saved CodeCompanion Chats | <c-d>: delete", -- Set the prompt title for the picker
      cwd = save_folder:absolute(), -- Set the current working directory to the absolute path of the save folder
      attach_mappings = function(_, map) -- Attach custom mappings to the picker
        map("i", "<c-d>", function(prompt_bufnr) -- Map the <c-d> key in insert mode to delete the selected file
          local selection = t_action_state.get_selected_entry() -- Get the selected entry from the picker
          local filepath = selection.path or selection.filename -- Get the file path or filename of the selected entry
          os.remove(filepath) -- Remove the selected file
          t_actions.close(prompt_bufnr) -- Close the picker
          start_picker() -- Restart the picker to refresh the file list
        end)
        return true -- Return true to indicate that the mappings were successfully attached
      end,
    })
  end
  start_picker() -- Start the picker when the command is executed
end, {})

-- Save current chat, `CodeCompanionSave foo bar baz` will save as 'foo-bar-baz.md'
vim.api.nvim_create_user_command("CodeCompanionSave", function(opts) -- Create a new user command "CodeCompanionSave"
  local codecompanion = require("codecompanion") -- Import the codecompanion module
  local success, chat = pcall(function() -- Use pcall to safely call a function and capture any errors
    return codecompanion.buf_get_chat(0) -- Get the chat from the current buffer
  end)
  if not success or chat == nil then -- Check if the function call was not successful or the chat is nil
    vim.notify("CodeCompanionSave should only be called from CodeCompanion chat buffers", vim.log.levels.ERROR) -- Notify the user of the error
    return -- Exit the function early
  end
  if #opts.fargs == 0 then -- Check if no arguments were provided
    vim.notify("CodeCompanionSave requires at least 1 arg to make a file name", vim.log.levels.ERROR) -- Notify the user of the error
  end
  local save_name = table.concat(opts.fargs, "-") .. ".md" -- Concatenate the arguments with hyphens and add the .md extension
  local save_path = Path:new(save_folder, save_name) -- Create a new path object for the save file
  local lines = vim.api.nvim_buf_get_lines(0, 0, -1, false) -- Get all lines from the current buffer
  save_path:write(table.concat(lines, "\n"), "w") -- Write the lines to the save file
end, { nargs = "*" }) -- Allow the command to accept multiple arguments

-- Define the prefix for key mappings
local prefix = "<leader>aC"

-- Create an autocommand to handle the "CodeCompanionChatAdapter" event
vim.api.nvim_create_autocmd("User", {
  pattern = "CodeCompanionChatAdapter",
  callback = function(args)
    -- Check if the adapter data is nil or empty
    if args.data.adapter == nil or vim.tbl_isempty(args.data) then
      return
    end
    -- Set the global variable for the adapter name
...
return {
  {
    ...
    init = function()
      vim.cmd([[cab cc CodeCompanion]]) -- Create a command alias for CodeCompanion
    end,
    config = function(_, options)
      require("codecompanion").setup(options) -- Setup CodeCompanion with provided options
      ...
      vim.api.nvim_create_autocmd({ "User" }, {
        ...
        callback = function(request)
          ...
          if request.match == "CodeCompanionRequestFinished" then
            spinner.hide() -- Hide spinner when request finishes
          end
        end,
      })
    end,
